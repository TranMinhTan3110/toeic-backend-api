...
public class OpenAiService : IAiService
{
    ...
    public OpenAiService(HttpClient httpClient, IConfiguration configuration, IMemoryCache cache)
    {
...
- [Giải thích cấu trúc của câu mẫu đề xuất ở trên để học viên học theo]";
        ...
        var result = await CallOpenAiAsync(prompt, 4000); 
...
Thử thách cho từ '{word}':";
        ...
        return await CallOpenAiAsync(prompt, 1000);
    }
...
Nội dung biên soạn lý thuyết cho '{topicTitle}':";
        ...
        return await CallOpenAiAsync(prompt, 4000);
    }
...
Nội dung JSON của bộ câu hỏi thực hành cho '{topicTitle}':";
        ...
        return await CallOpenAiAsync(prompt, 4000, isJson: true);
    }
        ...
        string? mimeType = null)
    {
        ...
        var prompt = $@"
Bạn là giám khảo TOEIC Speaking chuyên nghiệp. Hãy chấm điểm câu trả lời của học viên bằng cách phân tích bản dịch transcript được cung cấp và SO SÁNH với bài mẫu và đề bài.

- Part / Task: {taskNumber}
- Đề bài / Prompt: {taskPrompt}
- Bài mẫu tham chiếu:
{samplesBlock}
- Câu trả lời học viên (bản dịch transcript): {userTranscript}

YÊU CẦU ĐÁNH GIÁ CHẤT LƯỢNG NÓI CHUYÊN SÂU (ĐÁNH GIÁ QUA TEXT TRANSCRIPT):
1. Phân tích ngữ pháp, từ vựng, tính liên kết và độ trôi chảy logic của văn bản.
2. Đánh giá ước lượng Phát âm (Pronunciation) và Lưu loát (Fluency) dựa trên độ chính xác và mạch lạc của transcript so với bài mẫu.
...
{{
  ...
  ""transcript"": ""<văn bản chính xác bạn nghe được từ file ghi âm của học viên. Nếu im lặng hoặc không rõ, ghi '(Không nhận diện được giọng nói)'>""
        ...
        var raw = await CallOpenAiAsync(prompt, 2000, isJson: true);
...
{{
  ...
  ""suggestedImprovement"": ""<bài viết mẫu cải tiến nâng cấp từ chính bài làm của học viên, giúp câu văn tự nhiên và chuyên nghiệp hơn>""
        ...
        var raw = await CallOpenAiAsync(prompt, 3000, isJson: true);
    ...
    private async Task<string> CallOpenAiAsync(string prompt, int maxTokens, bool isJson = false)
    {
        Console.WriteLine($"[OPENAI DEBUG] Đang gọi AI với maxTokens: {maxTokens}, isJson: {isJson}");
        ...
        while (retryCount <= maxRetries)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Log response (cắt ngắn nếu quá dài khi in console)
                    Console.WriteLine($"[OPENAI RESPONSE] {(responseString.Length > 200 ? responseString.Substring(0, 200) + "..." : responseString)}");
                    ...
                    if (root.TryGetProperty("choices", out var choices) && choices.GetArrayLength() > 0)
                    {
                        ...
                        {
                            return contentProp.GetString()?.Trim() ?? "AI không trả về văn bản.";
                        }
                    }
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.TooManyRequests)
                {
                    ...
                    if (retryCount <= maxRetries)
                    {
                        ...
                        Console.WriteLine($"[OPENAI] Gặp lỗi 429 (TooManyRequests). Đang thử lại lần {retryCount} sau {delay}ms...");
                ...
                Console.WriteLine($"[OPENAI ERROR] {response.StatusCode}: {errorMsg}");
